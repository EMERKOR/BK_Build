...
def main():
    """Main execution function for Ball Knower demo."""
    print("\n" + "="*80)
    ...
    # Compute EPA margin if EPA columns exist
    if 'EPA/Play' in team_ratings.columns and 'EPA/Play Against' in team_ratings.columns:
        ...
        team_ratings['epa_margin'] = 0.0

    print(f"\nTop 10 Teams by nfelo:")
    # Use actual column names from merged data
    display_cols = ['team', 'nfelo', 'Ovr.']
    # Add EPA columns if they exist
    if 'EPA/Play' in team_ratings.columns:
        display_cols.append('EPA/Play')
    if 'EPA/Play Against' in team_ratings.columns:
        display_cols.append('EPA/Play Against')
