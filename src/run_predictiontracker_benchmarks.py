...
def run_benchmark(
    ...
    print("\n--- Mean Absolute Error vs Actual Game Margin ---")

    if 'mae_pt_vs_actual' in summary:
        print(f"  PredictionTracker: {summary['mae_pt_vs_actual']:.2f} points")
    else:
        print(f"  PredictionTracker: N/A")

    if 'mae_vegas_vs_actual' in summary:
        print(f"  Vegas closing:     {summary['mae_vegas_vs_actual']:.2f} points")
    else:
        print(f"  Vegas closing:     N/A")

    if 'mae_bk_vs_actual' in summary:
        print(f"  Ball Knower v1.2:  {summary['mae_bk_vs_actual']:.2f} points")
    else:
        print(f"  Ball Knower v1.2:  N/A (no BK predictions in dataset)")
    ...
    sample_cols = ['season', 'week', 'away_team', 'home_team', 'home_margin',
                   'vegas_closing_spread', 'pt_spread', 'mae_pt_vs_actual', 'mae_vegas_vs_actual']
